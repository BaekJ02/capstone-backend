...
- 미국주식 호가창 WebSocket 버그 수정
  - `handleOverseasOrderBook`: `fields[0]` = `DNASMU` 형태 → `substring(4)`로 순수 심볼(`MU`) 추출
  - `subscribeOverseasOnly`: `kisWebSocketClient.subscribeOverseas()` 호출 누락 수정 (중복 방지 포함)
  - `subscribeDomesticOnly`: `kisWebSocketClient.subscribe()` 호출 누락 수정 (중복 방지 포함)
  - 미국 호가창 REST API: `GET /api/stocks/orderbook/overseas/{symbol}?exchange=` → 빈 OrderBookDto 반환 (KIS REST 미제공)
...
- 시장 순위: `MarketRankingController`, `MarketRankingService`, `RankingItemDto` 구현 (인증 불필요)
  ...
  - 프론트 거래대금 표시: `fmtVolume()` 헬퍼로 조/억 단위 축약 표시, 해외 VOLUME은 tamt(달러) × 환율로 원화 환산 후 표시, 컬럼 헤더 "거래대금"으로 통일
...
- AI 시장 뉴스: `MarketNewsService`, `MarketNewsController`, `MarketNewsDto` 신규 생성
  - `GET /api/market/news/overseas` (인증 불필요, 캐싱 응답)
  - `GET /api/market/news/domestic` (인증 불필요, 캐싱 응답)
  - 해외: KIS 해외뉴스종합 API (tr_id: HHPSTH60100C1, NATION_CD=US) 제목 10건 + NAS 거래대금 상위 20종목
  - 국내: KIS 종합 시황/공시 API (tr_id: FHKST01011800) 제목 10건 + 국내 거래대금 상위 20종목
    - 공시(F/G/N) 및 인포스탁(7) 필터링, 언론사 뉴스만 수집
  - 뉴스 + 종목 데이터 → Claude Haiku 분석 → JSON 구조화
  - 응답 필드: updatedAt, weather(SUNNY/CLOUDY/RAINY/STORM), headlines, positive(sector+reason+stocks), negative(sector+reason+stocks), summary
  - StockDto: symbol, name, changePercent / SectorDto: sector, reason, stocks
  - `@PostConstruct`: 서버 시작 시 국내/해외 각각 1회 자동 갱신
  - `@Scheduled` cron: 매일 08:00/14:00/16:00/20:00, 22:30, 05:00 갱신
  - build.gradle: `jackson-databind` 추가 / AppConfig: `ObjectMapper` 빈 추가
...
- 데일리 퀴즈: `QuizService`, `QuizController`, `DailyQuiz`, `QuizResult` 도메인 구현
  - GET /api/quiz/today (로그인 필요) → 오늘의 퀴즈 반환
  - POST /api/quiz/submit (로그인 필요) → 정답 제출 + 보상 지급
  - 매일 00시 `@Scheduled(cron = "0 0 0 * * *")`으로 퀴즈 자동 생성
  - `@PostConstruct`로 서버 시작 시 오늘 퀴즈 없으면 즉시 생성
  - Claude Haiku로 OX/4지선다 랜덤 생성, DB(`daily_quiz`)에 저장
  - 정답 시 국내 MARKET_CAP TOP20 + 해외 MARKET_CAP TOP20 중 랜덤 1주 보유종목 자동 지급
  - 현재가 조회 후 `Holding` 테이블에 추가 (기존 보유 시 평균가 재계산)
  - `quiz_result` 테이블에 정답 여부 + 보상 정보 기록
  - 개발 모드: `alreadySolved` 항상 false (하루 여러 번 가능)
  - DB 테이블: `daily_quiz`(id, quizDate, question, type, options, answer, explanation), `quiz_result`(id, user_id, quizDate, isCorrect, rewardSymbol, rewardName, rewardMarket, rewardPrice, createdAt)
  - 신규 파일: `domain/DailyQuiz.java`, `domain/QuizResult.java`, `repository/DailyQuizRepository.java`, `repository/QuizResultRepository.java`, `dto/QuizDto.java`, `dto/QuizResultDto.java`, `service/QuizService.java`, `controller/QuizController.java`
  - `HoldingRepository`에 `findByUserIdAndSymbol(Long userId, String symbol)` 추가
...
- 장외 시간 REST 폴링 간격 개선 완료 (5초/200ms) — 에러 빈도 감소
