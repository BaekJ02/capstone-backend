...
세션 기반 인증을 사용하므로 모든 요청에 `credentials: 'include'` 를 포함해야 해요.
...
> 로그인 성공 시 세션 쿠키가 자동으로 저장돼요. 이후 요청에 자동으로 포함돼요.
...
3. **모든 요청에 `withCredentials: true` 필수예요.** 없으면 로그인 세션이 유지되지 않아요.
4. **로그인 후에만** 매매, 잔고, 보유종목, 주문내역 API를 사용할 수 있어요. 미로그인 시 401 응답이 와요.
